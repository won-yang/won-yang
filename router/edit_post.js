...
router.post("/edit_ok", async (req, res) => {
    if(!permission.isLogin(req.session.passport)){
    ...
    if (admin_rows.length == 0) 
        ...
        await sendQuery(`UPDATE post SET title = ?, post_date = ? WHERE post_idx = ?`, [send_data.title, date_row[0].post_date, post_idx]);
    
    if(image_path != undefined){
        await sendQuery(`UPDATE thumbnail SET image_path = ? WHERE post_idx = ?`, [image_path.attrs.src, post_idx]);   
    }
    await sendQuery(`UPDATE tag SET main_gate = ?, west_gate = ?, east_gate = ?, etc_gate = ? WHERE post_idx = ?`, [gate.main_gate, gate.west_gate, gate.east_gate, gate.etc_gate, post_idx]);
    await sendQuery(`UPDATE options SET refrigerator = ?, desk = ?, wifi = ?, washing_machine = ?,
                    gas_stove = ?, microwave = ?, cctv = ?, closet = ?, bed = ?, tv = ?,
                    public_washing_machine = ?, public_kitchen = ?, elevator = ?, air_conditioner = ?, door_lock = ?  WHERE post_idx = ?`, [send_data.refrigerator, send_data.desk, send_data.wifi, send_data.washing_machine, send_data.gas_stove, send_data.microwave, send_data.cctv, send_data.closet, send_data.bed, send_data.tv, send_data.public_washing_machine, send_data.public_kitchen, send_data.elevator, send_data.air_conditioner, send_data.door_lock, post_idx])
    await sendQuery(`UPDATE post_content SET contact = ?, contract_period = ?, address = ?, monthly_rent = ?, content = ?, deposit = ? WHERE post_idx = ?`, 
                    [send_data.contact, send_data.contract_period, send_data.address, send_data.monthly_rent, send_data.content, send_data.deposit, post_idx]);
